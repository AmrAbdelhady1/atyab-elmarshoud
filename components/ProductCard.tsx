...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const handleWishList = async (id: number) => {
    const url = isLiked ? "remove-favourites" : "add-favourites";
  ...
  return (
    <div
      className={`${wrapperClassName} flex flex-col gap-8 items-center group`}
    >
      ...
      <div className="flex gap-0 w-full justify-center">
        {!product?.is_favorite && (
          <button
            ...
            style={{
              backgroundImage:
                "linear-gradient(135deg, #339994 50%, transparent 51%)",
            }}
