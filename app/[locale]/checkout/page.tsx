...
async function getPaymentMethods(locale: string, token: string) {
  try {
    const response = await fetch(`${BASE_URL_NO_API}/payment_methods/7`, {
      headers: {
        locale,
        Authorization: `Bearer ${token}`,
      },
...
const page = async ({ params: { locale } }: { params: { locale: string } }) => {
  ...
  console.log("amr", paymentMethods);
  ...
  return <CheckoutPage addressData={addressData} checkoutData={checkoutData} paymentMethods={paymentMethods} />;
};
